...
2.  **👥 Выбор команды и роли.** Присоединяясь к комнате, игроки могут занять любую свободную роль в любой команде:
    -   👑 Капитан (Spymaster).
    -   🕵️ Оперативник (Operative).

3.  **⏱️ Запуск игры.** Как только комната полностью заполняется, автоматически запускается обратный отсчет (например, 10 секунд) до старта игры.

4.  **🎲 Автоназначение капитанов.** Если по окончании обратного отсчета в какой-либо команде не назначен капитан, система **🎲 случайным образом** выбирает капитана из числа игроков этой команды. Остальные игроки в команде становятся оперативниками.

5.  **🃏 Генерация поля и распределение ролей.** После запуска игры всем игрокам открывается игровое поле 5x5 со словами. Однако отображение информации различается в зависимости от роли:
    ...
    > [**Референс** Пример: что видит капитан vs оперативник](https://github.com/rolling-scopes-school/tasks/blob/master/stage2/tasks/rs-tandem/examples/02-codenames-game/variant-a/game-engine.md#пример-что-видит-капитан-vs-оперативник)

    Распределение цветов на поле зависит от того, какая команда ходит первой, и генерируется **🎲 случайным образом**:
...
> [**Референс** Жизненный цикл комнаты](https://github.com/rolling-scopes-school/tasks/blob/master/stage2/tasks/rs-tandem/examples/02-codenames-game/variant-a/game-engine.md#жизненный-цикл-комнаты)
...
2.  **🕵️ Оперативник (Operative).**
    ...
    *   **🗳️ Участвует в обсуждении и голосовании.** Получив подсказку от своего капитана, оперативники обсуждают ее. Чтобы выбрать карточку для хода, они должны прийти к консенсусу через голосование.
...
3.  **🗳️ Фаза угадывания (Guess Phase).**
    *   Оперативники видят подсказку и обсуждают возможные варианты.
...
4.  **⚙️ Обработка клика (на стороне сервера).**
    *   Карточка открывается для всех: ее статус меняется на `revealed`.
    *   В зависимости от цвета карточки определяется дальнейшее действие:
        *   **✅ Своя карточка:** игра переходит в фазу проверки знаний (**Check Phase**).
        *   **❌ Чужая карточка или нейтральная:** ход команды немедленно завершается, право хода переходит к противнику. Очко не начисляется.
        *   **💣 Бомба:** игра мгновенно завершается победой команды противника.

    ```mermaid
        flowchart LR
            A[🃏 Карточка открыта<br>status = revealed] --> B{🎨 Определение цвета}
            
            B -->|✅ Своя| C[❓ Переход в Check Phase]
            B -->|❌ Чужая или нейтральная| D[⏭️ Ход команды завершен<br>Право хода переходит к противнику]
            B -->|💣 Бомба| E[🏁 Игра завершена<br>Победа команды противника]
...
> [**Референс** User Flow](https://github.com/rolling-scopes-school/tasks/blob/master/stage2/tasks/rs-tandem/examples/02-codenames-game/variant-a/check-phase.md#user-flow)
...
> [**Референс** Ход игры (Clue -> Guess -> Check)](https://github.com/rolling-scopes-school/tasks/blob/master/stage2/tasks/rs-tandem/examples/02-codenames-game/variant-a/game-engine.md#ход-игры-clue---guess---check)

### 3.4. ❓ Фаза Check (проверка знаний)

> [**Референс** Check Phase: Проверка знаний](https://github.com/rolling-scopes-school/tasks/blob/master/stage2/tasks/rs-tandem/examples/02-codenames-game/variant-a/check-phase.md)
...
2.  **💥 Выбор карточки-убийцы (бомбы).** Если команда в свой ход открывает карточку **💣 убийцы (бомбы)**, игра заканчивается ее **мгновенным поражением**. Победа присуждается команде противника, независимо от оставшихся на поле карточек.
    > [**Референс** Пример 2: Codenames — Interview Edition](https://github.com/rolling-scopes-school/tasks/blob/master/stage2/tasks/rs-tandem/examples/02-codenames-game.md)
